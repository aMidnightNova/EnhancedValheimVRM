...
internal static class Program
{
    ...
    private static void Main()
    {
        ...
        Check(AttachmentMath.TryMapSocket(parent, turn, identity, captured, 2, out var position), "Socket mapping failed");
        ...
        foreach (float avatarRatio in new[] { 0.5f, 1, 2 })
        {
            ...
            Check(AttachmentMath.TryMapSocket(parent, identity, identity, offset, avatarRatio, out position), "Import-scale mapping failed");
            Near(math.transform(parent, position) - new float3(3, 4, 5), offset * avatarRatio, "Avatar ratio must apply once");
            for (int repeat = 0; repeat < 3; repeat++)
            {
                Check(AttachmentMath.TryMapSocket(parent, identity, identity, offset, avatarRatio, out var repeated), "Repeated mapping failed");
        ...
        var rotatedCapture = AttachmentMath.ToSocketOffset(math.transform(bodyMatrix, sourcePosition),
            math.transform(bodyMatrix, sourceSocket), math.mul(worldTurn, gripRotation));
        ...
        Check(AttachmentMath.TryMapSocket(target, turn, gripRotation, firstCapture, 1, out var firstPosition), "Base target mapping");
        var movedTarget = math.mul(bodyMatrix, target);
        Check(AttachmentMath.TryMapSocket(movedTarget, math.mul(worldTurn, turn), gripRotation,
            rotatedCapture, 1, out var movedPosition), "Moved target mapping");
        ...
        Check(AttachmentMath.TryLocalScale(parent, gripRotation, new float3(1), out var localScale), "Scale conversion failed");
        Near(localScale, new float3(1f / 3, 0.5f, 0.25f), "Rotated local axes use the correct parent scale");
        Near(ColumnLengths(math.mul(parent, float4x4.TRS(float3.zero, gripRotation, localScale))), new float3(1), "World equipment scale");

        // Nested nonuniform scales can produce shear: preserve requested axis lengths.
        parent = math.mul(float4x4.Scale(new float3(2, 3, 4)), float4x4.TRS(new float3(5), worldTurn, new float3(0.5f, 2, 1)));
        Check(AttachmentMath.TryLocalScale(parent, gripRotation, new float3(0.5f), out localScale), "Nested scale conversion failed");
        Near(ColumnLengths(math.mul(parent, float4x4.TRS(float3.zero, gripRotation, localScale))), new float3(0.5f), "Nested world scale");
        Check(AttachmentMath.TryMapSocket(parent, worldTurn, gripRotation, captured, 1, out position), "Nested socket mapping failed");
        Near(math.transform(parent, position) - parent.c3.xyz,
            math.rotate(math.mul(worldTurn, gripRotation), captured), "Nested displacement mapping");
        ...
        foreach (float ratio in new[] { 0.5f, 1, 2 })
        {
            ...
            Check(AttachmentMath.TryLocalScale(parent, identity, new float3(1), out var socketScale), "Socket scale failed");
            var socketMatrix = math.mul(parent, float4x4.Scale(socketScale));
            Check(AttachmentMath.TryLocalScale(socketMatrix, identity, new float3(ratio), out var itemScale), "Equipment scale failed");
            Near(ColumnLengths(math.mul(socketMatrix, float4x4.Scale(itemScale))), new float3(ratio), "Equipment double-scaled");
        }
        ...
        Near(AttachmentMath.PlaceBone(handPosition, turn, boneOffset), handPosition + worldOffset + math.rotate(turn, correction), "Dual-wield space conversion");
        var rotatedBoneOffset = AttachmentMath.ToBoneOffset(math.mul(worldTurn, turn), math.rotate(worldTurn, worldOffset), correction);
        ...
        Near(AttachmentMath.PlaceBone(math.rotate(worldTurn, handPosition), math.mul(worldTurn, turn), boneOffset),
            math.rotate(worldTurn, AttachmentMath.PlaceBone(handPosition, turn, boneOffset)), "Dual-wield animation rotation");
        ...
        foreach (float avatarRatio in new[] { 0.5f, 0.9043996f, 2f })
        {
            ...
            Check(AttachmentMath.TryMapItemOffset(socketMatrix, worldTurn, configuredOffset, avatarRatio, out var itemPosition), "Back item offset conversion");
            Near(math.transform(socketMatrix, itemPosition), socketPosition + math.rotate(worldTurn, configuredOffset * avatarRatio), "Back item distance/direction must not depend on import scale");
            Check(AttachmentMath.TryMapItemOffset(socketMatrix, worldTurn, configuredOffset, avatarRatio, out var repeatedPosition), "Repeated back offset conversion");
            Near(repeatedPosition, itemPosition, "Holstering must not accumulate offset");
        }
        Check(!AttachmentMath.TryMapItemOffset(float4x4.Scale(float3.zero), identity, new float3(0, 0.13f, 0), 1, out position), "Singular back socket accepted");
        ...
        Check(AttachmentMath.TryMapItemOffset(equipParent, turn, equipBase + new float3(0, 0.13f, 0), 0.5f, out var equipPosition), "Prefab offset mapping");
        Near(math.transform(equipParent, equipPosition), new float3(4, 5.065f, 5.985f), "Prefab and configured offsets must both survive scale conversion");
        ...
        Near(math.rotate(smoother.Sample(identity, float3.zero, 0, 0, 0.02f), forward), forward, "First turn sample must snap");
        Near(math.rotate(smoother.Sample(sixDegrees, float3.zero, 0.02f, 0.02f, 0.02f), forward), forward, "New fixed tick starts at previous rotation");
        Near(math.rotate(smoother.Sample(sixDegrees, float3.zero, 0.02f, 0.025f, 0.02f), forward),
            math.rotate(quaternion.RotateY(math.radians(1.5f)), forward), "Quarter tick interpolation");
        Near(math.rotate(smoother.Sample(sixDegrees, float3.zero, 0.02f, 0.03f, 0.02f), forward),
            math.rotate(quaternion.RotateY(math.radians(3)), forward), "Half tick interpolation");
        Near(math.rotate(smoother.Sample(sixDegrees, float3.zero, 0.02f, 0.04f, 0.02f), forward),
            math.rotate(sixDegrees, forward), "Completed interpolation");
        Near(math.rotate(smoother.Sample(sixDegrees, float3.zero, 0.04f, 0.06f, 0.02f), forward),
            math.rotate(sixDegrees, forward), "Stopped turn must settle without oscillation");

        smoother.Reset();
        smoother.Sample(quaternion.RotateY(math.radians(359)), float3.zero, 0, 0, 0.02f);
        Near(math.rotate(smoother.Sample(quaternion.RotateY(math.radians(1)), float3.zero, 0.02f, 0.03f, 0.02f), forward),
            forward, "Turn through north must take short path");
        Near(math.rotate(smoother.Sample(turn, new float3(100, 0, 0), 0.04f, 0.045f, 0.02f), forward),
            math.rotate(turn, forward), "Teleport must discard old rotation");
        Near(math.rotate(smoother.Sample(identity, new float3(100, 0, 0), 1, 1, 0.02f), forward), forward, "Long hitch must discard stale samples");
        Near(math.rotate(smoother.Sample(sixDegrees, new float3(100, 0, 0), 0, 0, 0.02f), forward),
            math.rotate(sixDegrees, forward), "Clock reset must discard stale samples");
        smoother.Reset();
        Near(math.rotate(smoother.Sample(turn, float3.zero, 0, 0, 0), forward), math.rotate(turn, forward), "Invalid fixed step must snap");
        ...
        Near(math.rotate(math.mul(turnCorrection, math.mul(sixDegrees, handAxes)), forward),
            math.rotate(math.mul(renderedTurn, handAxes), forward), "Special weapon must follow rendered root rotation");

        Check(!AttachmentMath.TryMapSocket(float4x4.Scale(new float3(0)), identity, identity, captured, 1, out position), "Singular parent accepted");
        Check(!AttachmentMath.TryMapSocket(float4x4.identity, identity, identity, captured, float.NaN, out position), "NaN ratio accepted");
        Check(!AttachmentMath.TryMapSocket(float4x4.identity, identity, identity, captured, 0, out position), "Zero ratio accepted");
        Check(!AttachmentMath.TryLocalScale(float4x4.Scale(new float3(0)), identity, new float3(1), out localScale), "Zero parent scale accepted");
