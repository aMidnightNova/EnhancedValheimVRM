...
internal static class OutfitChecks
{
    public static async Task Run(string root, Action<bool, string> check)
    {
        ...
        var importedText = string.Join("\n", new[]
        {
            ...
            "[Casual]", "Default=False", "mesh:Jacket=False", "blendshape:Body:Tuck=0.92", "",
            "[Beach]", "Default=False", "mesh:Shorts=False", "mesh:Jacket=False", "blendshape:Body:Tuck=67", ""
        });
        var imported = OutfitConfig.Parse(importedText);
        check(imported.Outfits.Count == 3 && imported.Default.Name == "Default", "Three-outfit file with a flagged default");
        ...
        var renamed = OutfitConfig.Parse("[Anything]\nDefault=False\n[My usual clothes]\nDefault=True\nmesh:Body=True\n");
        check(renamed.Default.Name == "My usual clothes", "Default must come from flag, not section name or order");
        foreach (string invalid in new[] {
            "[A]\nDefault=False", "[A]\nDefault=True\n[B]\nDefault=True", "[A]\nDefault=True\n[a]\nDefault=False",
            "[A]\nDefault=True\nblendshape:Body:Test=NaN", "[A]\nDefault=True\nblendshape:Body:Test=101",
            "[A]\nDefault=True\nmesh:Body=perhaps", "[A]\nDefault=True\nmesh:Body=True\nmesh:Body=False",
            "[A]\nDefault=True\ndefault=False", "[A]\nDefault=True\nblendshape:Body:=25" })
        {
            bool rejected = false;
            try { OutfitConfig.Parse(invalid); } catch (InvalidDataException) { rejected = true; }
        ...
        check(BundleCrypto.Version(BundleCrypto.PackProfile("", importedText.Replace("Default=True", "Default=False")), key) != originalVersion,
