...
namespace EnhancedValheimVRM
{
    public class ConsoleCommands
    {
        ...
        private const string Usage = "/vrm outfit list | next | reload | generate | set <name>; /vrm toggle <mesh>; /vrm blend <shape> <1-100>; /vrm settings reload | auto on | auto off";
        ...
        static ConsoleCommands()
        {
            ...
            Vrm = new Terminal.ConsoleCommand("vrm", Usage, args =>
            {
                ...
                {
                    bool twoHanded = args.Args.Length == 4 && args.Args[2].Equals("weapons", StringComparison.OrdinalIgnoreCase) &&
                        args.Args[3].Equals("twohanded", StringComparison.OrdinalIgnoreCase);
                    if (args.Args.Length != 3 && !twoHanded)
                    { args.Context.AddString(DevUsage); return; }
                    if (args.Args[2].Equals("off", StringComparison.OrdinalIgnoreCase)) args.Context.AddString(VrmController.SetLocalEnabled(false));
                    else if (args.Args[2].Equals("on", StringComparison.OrdinalIgnoreCase)) args.Context.AddString(VrmController.SetLocalEnabled(true));
                    ...
                    else args.Context.AddString(DevUsage);
                    return;
                }
                ...
                if (group.Equals("settings", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (!auto && (args.Args.Length != 3 || !action.Equals("reload", StringComparison.OrdinalIgnoreCase)))
                    {
                ...
                if (!group.Equals("outfit", StringComparison.OrdinalIgnoreCase) &&
                    !group.Equals("toggle", StringComparison.OrdinalIgnoreCase) && !group.Equals("blend", StringComparison.OrdinalIgnoreCase))
                {
                    args.Context.AddString(Usage);
                    return;
                }

                var outfits = Player.m_localPlayer?.GetVrmInstance()?.GetGameObject()?.GetComponent<OutfitController>();
                if (outfits == null)
                {
                    args.Context.AddString("Load a character with a VRM first.");
                    return;
                ...
                {
                    if (args.Args.Length < 4 || !float.TryParse(args.Args.Last(), System.Globalization.NumberStyles.Float,
                        System.Globalization.CultureInfo.InvariantCulture, out var weight) || float.IsNaN(weight) || weight < 1 || weight > 100)
                    { args.Context.AddString("Usage: /vrm blend <blendshape> <1-100>"); return; }
                    string shape = string.Join(" ", args.Args.Skip(2).Take(args.Args.Length - 3)).Trim().Trim('"');
                    args.Context.AddString(outfits.Override(shape, true, weight, true) ? "Blendshape: " + shape + " = " + weight : "Unknown blendshape: " + shape);
                    return;
                }
