...
namespace EnhancedValheimVRM
{
    ...
    public sealed class AttachmentReference : MonoBehaviour
    {
        ...
        public void Capture()
        {
            ...
            _offsetInSocketAxes = AttachmentMath.ToSocketOffset(AttachmentTransforms.Vector(transform.parent.position),
                AttachmentTransforms.Vector(transform.position), AttachmentTransforms.Rotation(transform.rotation));
        ...
        public bool Reparent(Transform parent, Quaternion rotation, float ratio, Vector3? scaleOverride)
        {
            ...
            if (!AttachmentMath.TryMapSocket(matrix, AttachmentTransforms.Rotation(parent.rotation),
                    AttachmentTransforms.Rotation(rotation), _offsetInSocketAxes, ratio, out var position)) return false;
            if (!AttachmentMath.TryLocalScale(matrix, AttachmentTransforms.Rotation(rotation),
                    AttachmentTransforms.Vector(_worldScale), out var scale)) return false;
    ...
    {
        public static float3 Vector(Vector3 value) { return new float3(value.x, value.y, value.z); }
        public static Vector3 Vector(float3 value) { return new Vector3(value.x, value.y, value.z); }
        public static quaternion Rotation(Quaternion value) { return new quaternion(value.x, value.y, value.z, value.w); }
