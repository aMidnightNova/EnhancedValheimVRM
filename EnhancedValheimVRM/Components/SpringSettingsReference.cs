...
namespace EnhancedValheimVRM
{
    public sealed class SpringSettingsReference : MonoBehaviour
    {
        ...
        private readonly Dictionary<VRM10SpringBoneJoint, Vector2> _vrm1 = new Dictionary<VRM10SpringBoneJoint, Vector2>();
        public void Apply(VrmSettings settings)
        {
            foreach (var spring in GetComponentsInChildren<VRMSpringBone>(true))
            {
                if (!_vrm0.TryGetValue(spring, out var original)) _vrm0[spring] = original = new Vector2(spring.m_stiffnessForce, spring.m_gravityPower);
            ...
            {
                if (!_vrm1.TryGetValue(joint, out var original)) _vrm1[joint] = original = new Vector2(joint.m_stiffnessForce, joint.m_gravityPower);
