...
namespace EnhancedValheimVRM
{
    ...
    {
        private static readonly System.Type EquipmentVisual =
            typeof(VisEquipment).Assembly.GetType("IEquipmentVisual", true);
        ...
        private static bool Prefix(VisEquipment __instance,
            ...
            int quality = 0)
        {
            if (!__instance.m_isPlayer || !__instance.TryGetComponent<Player>(out var player) ||
                player.GetVrmInstance()?.GetGameObject() == null)
                return true;
            ...
            {
                if (backAttach && child.name == "attach_back")
                {
                    attachment = child;
                    break;
                }

                if (child.name == "attach" || (!backAttach && child.name == "attach_skin"))
                {
                    attachment = child;
                    break;
                }
            ...
            foreach (var scaler in instance.GetComponentsInChildren<ParticleIntensityScaler>())
                scaler.SetQuality(quality);
            instance.transform.SetParent(attachment.name == "attach_skin"
                ? __instance.m_bodyModel.transform.parent
                : joint);
